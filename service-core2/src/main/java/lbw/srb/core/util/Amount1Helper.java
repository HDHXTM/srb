...
public class Amount1Helper {
    ...
    public static Map<Integer, BigDecimal> getPerMonthPrincipal(BigDecimal invest, BigDecimal yearRate, int totalMonth) {
        ...
        for (Map.Entry<Integer, BigDecimal> entry : mapInterest.entrySet()) {
            BigDecimal subtract = monthIncome.subtract(entry.getValue());
            subtract = subtract.setScale(2, BigDecimal.ROUND_DOWN);
            mapPrincipal.put(entry.getKey(), subtract);
        }
        return mapPrincipal;
    }
