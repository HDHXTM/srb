...
//        String voteBindCode = (String)check.get("voteBindCode");
...
//同步设置
    ...
    public void setLendItemStartDateByLendId(Long id, LocalDate start, LocalDate end) {
        ...
        baseMapper.selectBatchIds(lendItems);
    }
...
//            lend.setLendEndDate(lend.getLendStartDate().plusMonths(lend.getPeriod()));
//        }
//        lendService.updateById(lend);
...
//获取投资人的bindCode
        ...
        wrapper.select("bind_code").eq("id",userId);
        String bindCode = (String) userInfoMapper.selectObjs(wrapper).get(0);

        //获取借款人的bindCode
        QueryWrapper<UserInfo> wrapper1 = new QueryWrapper<>();
        wrapper1.select("bind_code").eq("id",lend.getUserId());
        String benefitBindCode = (String) userInfoMapper.selectObjs(wrapper1).get(0);
